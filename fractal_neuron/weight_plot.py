...
ax.errorbar(angles, np.nanmean(lowi, axis = 0), yerr = np.nanstd(fps, axis = 1)/np.sqrt(10), ls = '', capsize = 5, color = colors[2], label = 'Increase ')
ax.scatter(angles, np.nanmean(lowi, axis = 0), color = colors[2])

ax.errorbar(angles, np.nanmean(medi, axis = 0), yerr = np.nanstd(fps, axis = 1)/np.sqrt(10), ls = '', capsize = 5, color = colors[1], label = 'Initial ')
ax.scatter(angles, np.nanmean(medi, axis = 0), color = colors[1])

ax.errorbar(angles, np.nanmean(high, axis = 0), yerr = np.nanstd(fps, axis = 1)/np.sqrt(10), ls = '', capsize = 5, color = colors[0], label = 'Decrease ')
...
ax.legend(title = r'Synaptic weight shift')
ax.set(xlabel = 'Tuning angle [deg]', ylabel = 'Firing frequency [Hz]', title = 'Abstract neuron morphology')
plt.tight_layout()
...
def fn(FRA, FRB, eFRB, eFRA):
    ...
    ax1.plot(angles, FRA + minuit_chi2.values['a'], color = 'Green', label = 'Addiative fit') 
    ...
    ax3.set(xlabel = 'Firing rate before (spikes/s)', ylabel = 'Firing range after (spikes/s)', title = 'Fit to data')
    ax1.legend()
    ax3.legend()
