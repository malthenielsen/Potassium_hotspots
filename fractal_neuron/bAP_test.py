...
def create_neuron(tL, dek, amp):
    ...
    dend_list = change_ek(dend_list, clust_dend, dek, 0)
    ...
    D_vec = h.Vector().record(trunk(1)._ref_v)
    D1 = h.Vector().record(dend_1(0.5)._ref_v)
    D11 = h.Vector().record(dend_11(0.5)._ref_v)
    D111 = h.Vector().record(dend_111(0.5)._ref_v)
    D1111 = h.Vector().record(dend_1111(0.5)._ref_v)

    return_list = [t_vec, V_vec, D1, D11, D111, D1111]
...
def auc(inn):
    ...
    inn += 60
    inn[inn < 0] = 0
    area = np.trapz(inn, dx = 0.001)
    return area

#  amps = np.linspace(0.5, .7, 20)
#  areas = np.zeros((3,20))
#  for j in range(20):
#      amp = amps[j]
#  #  amp = .6
#
#      res0 = create_neuron(500, 0, amp)
#      res6 = create_neuron(500, 6, amp)
#      res16 = create_neuron(500, 16, amp)
#      #  fig,ax = plt.subplots(2,1, figsize = (8,6))
#  #  ax[0].plot(res0[0], res0[1], color = 'black', label = 'soma')
#  #  ax[0].plot(res6[0], res6[1], color = 'black', label = 'soma')
#  #  ax[0].plot(res16[0], res16[1], color = 'black', label = 'soma')
#
#
#      for i in range(3,6):
#          area0 = auc(res0[i])
#          area6 = auc(res6[i])
#          area16 = auc(res16[i])
#          #  ax[1].plot(res0[0], res0[i], color = 'red', alpha = 1)
#          #  ax[1].plot(res6[0], res6[i], color = 'green', alpha = 1)
#          #  ax[1].plot(res16[0], res16[i], color = 'blue', alpha = 1)
#          #  print(area0)
#          #  print(area6)
#          #  print(area16)
#      areas[:,j] = [area0, area6, area16]
...
#  ax.plot(amps, areas[0,:])
#  ax.plot(amps, areas[1,:])
#  ax.plot(amps, areas[2,:])
#  plt.show()
