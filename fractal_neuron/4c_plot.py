...
def OSI(arr, ori):
    ...
    ori = np.deg2rad(ori)
    #  print(ori, arr)
    top = np.nansum(arr * np.exp(2*1j*ori/np.pi))
    F = top/np.nansum(arr)
...
def fn(FRA, FRB, eFRB, eFRA):
    ...
    ax1.errorbar(angles, FRB, yerr = eFRB, ls = '', color = 'blue')
    ...
    ax1.plot(angles, FRA + minuit_chi2.values['a'], color = 'black') 
    ...
    ax1.plot(angles, FRA*minuit_chi2.values['a'], color = 'red') 
    ax3.plot(FRA, FRA * minuit_chi2.values['a'])
