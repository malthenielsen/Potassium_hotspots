...
def change_ek(dend_list, clust_dend, ek,mix_ek):
    ...
    for dend_idx in clust_dend:
        ...
        cluster = int(2*idx//4) # To change ratio alter this value to test another ratio, as for now we are testing 1:1
        mixed = 0
...
def create_neuron(tL, dek, mix_dek, stim_alpha, not_keep, test = 0, ID = 1, AMP = 1, index = 0, mul = 1):
    ...
    dend_list = change_ek(dend_list, clust_dend, dek*0, mix_dek*0)
...
def run(i):
    ...
    for index, a in enumerate(angles):
        ...
        for idx, ek in enumerate(dek_r):
            ...
            #  VM, AUC, = create_neuron(t, dek_a(a), dek_r[idx], a, remix, 'ingle', ID, 0, index, 1)
            V_vec = create_neuron(t, dek_r[idx], dek_r[idx], a, remix, 'none', ID, 0, index, 1)
            soma_final.append(V_vec)

    np.save(f'./data_weight/soma_{i}', np.vstack(soma_final))
