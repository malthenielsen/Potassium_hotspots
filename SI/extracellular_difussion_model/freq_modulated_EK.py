...
for i in range(1,11):
    ...
    if i == center:
        ...
        rvs = np.random.uniform(-2*np.pi,2*np.pi, Nsyn)
    ...
    for t0, stim_alpha in enumerate(alphas):
        ...
        for idx in range(Nsyn):
            ...
            if i == center:
                ...
                weights[idx] = tuning_vm(np.rad2deg(rvs[idx])/2, 11, 0)
                #  weights[idx] = tuning_vm(rvs, 11, stim_alpha)


        weight = np.mean(weights)**1
        delays = np.random.uniform(0,1,200)
        delays = np.where(delays < 1/1000*weight*100)[0]
        delays += 300*t0 + 50
        print(weight, i, alpha, len(delays))
...
fire = np.hstack(fire)
print(fire)
print(sum(activity), 'number of sites', alpha)
...
def run(kind):
    ...
    for i in (range(N)):
        ...
        #  for j in range(10):
            #  if kind == 1:
        #grid[xsyn, ysyn] += 1800*activity*gauss(fire/dt, 65/dt, i)
        grid[xsyn, ysyn] += 4000*activity*gauss(fire/dt, 65/dt, i)
            #  if kind == 2:
...
if center == 5:
    np.save(f'freqmeasure_NAK_onoff_{int(decay)}_{Tstep}_{int(alpha)}', measure)
