...
def run(i):
    ...
    for a in angles:
        if a == 0:
            ...
            remix = 0
        for idx, ek in enumerate(dek):
            #  create_neuron(t, dek_a(a)*ek, dek_a_mix(a)*ek, a, remix, 'ingle', ID)
            t_vec, V_vec, stack = create_neuron(t, dek_a(a)*ek, dek_r[idx], a, remix, 'ingle', ID)
            stack_final.append(stack)
            soma_final.append(V_vec)
            print(ek, a, i)

    np.save(f'./data/3frac_5050/stack_{i}', np.vstack(stack_final))
