...
tl = [500]
angles = [0,5,10,15, 20, 30, 40,50, 80, 89]
...
def run(i):
    ...
    soma_final = []
    t = tl[0]
    tl = np.arange(100,900,100)
    time.sleep(i)
    ID = int(time.time())
    #  ID = np.random.randint(0,10000,1)[0]
    #  print(ID, i)
    for t in tl:
        if t == 100:
            remix = True
        ...
        for ek in dek:
            ...
            t_vec, V_vec, stack = create_neuron(t, dek_a(a)*ek, dek_a_mix(a)*ek, a, remix, 'ingle', ID)
            stack_final.append(stack)
            soma_final.append(V_vec)
            print(ek, a, i)

    np.save(f'./data/trunk_test/stack_{i}', np.vstack(stack_final))
    np.save(f'./data/trunk_test/soma_{i}', np.vstack(soma_final))
