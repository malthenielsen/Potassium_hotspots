...
def func(path):
    ...
    for l, fname in enumerate(fnames_trunk):
        ...
        for i in range(6,len(trunk)):
            #  if i in [10,12,15,16]:
                #  continue
            #  ax[i%3].plot(trunk[i][6500:11000], color = plt.cm.cool_r(i/N_t))
            #  ax[i%3].plot(trunk[i][9000:14000], color = plt.cm.cool_r(i/N_t) )
            #  ax[i%3].plot(trunk[i][:], color = plt.cm.cool_r((i//3)/(20-6)) )
            ax[i%3].plot(trunk[i][5000:8000], color = plt.cm.cool_r((i//3)/(20-6)) )
            #  print(N_t, i//3)
            ax[i%3].set_xticks([])
            #  ax[i%3].set_yticks([])
        ax[0].legend()
        ...
        ax[2].set_xticks(np.linspace(0,3000,5), np.linspace(0,200*0.6,5))
