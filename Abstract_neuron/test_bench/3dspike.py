...
#  clust_dend = np.array([30])
    ...
    D_vec = h.Vector().record(dend_list[dend_n]._ref_v)
...
def run(i):
    ...
    for index, a in enumerate(angles):
        ...
        for idx, ek in enumerate(dek_r):
            ...
            V_vec = create_neuron(t, dek_a(a), dek_r[idx], a, remix, 'single', ID, 0, index, 1)
            soma_final.append(V_vec)

    np.save(f'./data/dspike/soma_{i}', np.vstack(soma_final))
