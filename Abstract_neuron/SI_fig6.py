...
angles = np.append(angles, np.array([32, 34, 36, 38, 40, 50, 75, 90]))
...
mask = high > 0
p0_high_add, _ = curve_fit(fn_add, lowi[mask], high[mask], p0 = [1])
p0_high_mul, _ = curve_fit(fn_mul, lowi[mask], high[mask], p0 = [1])
p0_medi_add, _ = curve_fit(fn_add, lowi[mask], medi[mask], p0 = [1])
p0_medi_mul, _ = curve_fit(fn_mul, lowi[mask], medi[mask], p0 = [1])
print(p0_high_add)
print(p0_high_mul)
print(p0_medi_add)
print(p0_medi_mul)
...
ax[0].errorbar(angles, medi, yerr = np.nanstd(fps, axis = 1)/np.sqrt(10), ls = '', capsize = 5, color = colors[2])
ax[0].plot(angles, lowi*p0_medi_mul[0], color = 'red', label = 'Multiplicative')
ax[0].plot(angles, lowi + p0_medi_add[0], color = 'green', label = 'Addiative')
ax[0].legend()
...
ax[1].plot(lowi[mask], fn_add(lowi[mask], *p0_medi_add), color = 'green')
ax[1].plot(lowi[mask], fn_mul(lowi[mask], *p0_medi_mul), color = 'red')
...
ax2.set_xticks([0,1,2],['None', r'Low $\Delta E_K$', r'High $\Delta E_K$'] , rotation = 30)
plt.tight_layout()
...
def fn(FRA, FRB, eFRB, eFRA):
    ...
    ax1.errorbar(angles, FRB/FRA, yerr = mul_err, ls = '', color = 'red')

    ax1.set_ylim(0,3)
    ax2 = ax1.twinx()
    ax2.errorbar(angles, FRB - FRA, yerr = add_err, ls = '', marker = 'o', color = 'black')

    chi2_fit = Chi2Regression(fit, add_val, add_val, add_err)
    ...
    ax2.plot(angles, np.ones(len(angles))*minuit_chi2.values['a'], color = 'black') 
    ax3.plot(FRA, FRA + minuit_chi2.values['a'])

    chi2_fit = Chi2Regression(fit, mul_val, mul_val, mul_err)
    minuit_chi2 = Minuit(chi2_fit, a = 1)
    minuit_chi2.errordef = 1
    minuit_chi2.migrad()
    print(minuit_chi2.fval)
    pval = stats.chi2.sf(minuit_chi2.fval, len(angles[mask]))
    print(pval)
    ax1.plot(angles, np.ones(len(angles))*minuit_chi2.values['a'], color = 'red') 
    ax3.plot(FRA, FRA * minuit_chi2.values['a'])


    ax3.errorbar(FRA, FRB, yerr = eFRA, xerr =eFRB, ls = '')
    plt.show()

fn(low, high, np.nanstd(fps, axis = 1)/np.sqrt(10), np.nanstd(fps, axis = 1)/np.sqrt(10))
